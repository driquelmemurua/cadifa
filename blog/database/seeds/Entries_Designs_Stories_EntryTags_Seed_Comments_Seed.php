...
class Entries_Designs_Stories_EntryTags_Seed_Comments_Seed extends Seeder
{
	...
	public function run(){
		...
		$cantidad_max_tags = 10; /*cantidad maxima de entry_tags por cada entries*/
	    ...
	    for($i = 0; $i < $cantidad_entries; $i++){ /*Crear muchas entries*/
	        ...
	        $n_tags = rand(1,$cantidad_max_tags);

		    for ($j = 0; $j < $n_tags; $j++){ /*Crear varios entry_tags para cada entries*/
		    ...
		    for($k = 0; $k < $cantidad_comments; $k++){ /*Crear varios comments para cada entries*/
			...
			elseif($tipo_entries == 1): /*Entries seria un designs*/
    			...
    			for($image = 0; $image < $cantidad_images; $image++){
    			    $rand_images__created_at = rand($min_date, $max_date); /*la fecha de subida de cada imagen da igual para efectos de esta prueba */ 
		        	...
		        	'created_at' => $images_created_at, /*Las stories tienen mismas fechas que su entries asignado*/
